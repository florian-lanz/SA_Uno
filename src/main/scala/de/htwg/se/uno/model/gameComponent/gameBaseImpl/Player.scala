...
case class Player(handCards: List[Card] = List()) {
  ...
  def canPush(
    ...
    if revealedCard.value == Value.PlusTwo && card.value != Value.PlusTwo && revealedCardEffect > 0
    then false
    else if revealedCard.value == Value.PlusFour && card.value != Value.PlusFour && revealedCardEffect > 0
    then false
      ...
      def canPushRecursion(i: Int): Boolean = {
        if i < handCards.length then
          if handCards(
              i
            ).color == revealedCard.color && revealedCard.color != Color.Special && revealedCard.value != Value.PlusFour
          then false
  ...
  def pushCard(card: Card): Player = {
    ...
    def pushCardRecursion(i: Int): Int = {
      if i < handCards.length then
        if handCards(i).color == card.color && handCards(i).value == card.value
        then i
    ...
    if index != -1 then copy(handCards.take(index) ++ handCards.drop(index + 1))
    else copy()
  }
  ...
  def getCard(s: String): Option[Card] = {
    ...
    if index != -1 then Some(handCards(index))
    else None
  }
}
